...
class KerasLinear(KerasPilot):
    ...
    def inference(self, img_arr, other_arr):
        ...
        steering = outputs[0]
        if(steering * self.lastSteering<0):
            self.changeDirectionCount=self.changeDirectionCount+1;
        self.lastSteering=steering;
        currentTime=int(time.time())
        print(str(currentTime-self.startTime)+":"+str(self.changeDirectionCount)+":"+str(steering));
        throttle = outputs[1]
...
def core_cnn_layers(img_in, drop, l4_stride=1):
    ...
    """
    x = img_in
    x= Cropping2D(cropping=((20, 0), (0, 0)))(x)
    x = conv2d(24, 5, 2, 1)(x)
    x = Dropout(drop)(x)
    x = conv2d(32, 5, 2, 2)(x)
    x = Dropout(drop)(x)
